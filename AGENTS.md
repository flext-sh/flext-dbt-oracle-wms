...
**Package:** `flext_dbt_oracle_wms` · deps: `flext-core`, `flext-meltano`,
`flext-oracle-wms`

## Overview

Oracle WMS data transformation with dbt. Thin driver over `flext-meltano` dbt runner
(ADR-006).
...
- Workflow branches between model generation and optional transformations, returning a
  typed `WorkflowResult`. Connection profile is a typed `m.DbtOracleWms.*` model.
