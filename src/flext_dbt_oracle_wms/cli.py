...
class FlextDbtOracleWmsCliService:
    ...
    def handle_discover(
        self,
        _args: dict[str, t.GeneralValueType] | None = None,
    ) -> FlextResult[str]:
        """Handle discover command using flext-cli output."""
        try:
            config: dict[str, t.GeneralValueType] = (
                FlextDbtOracleWmsSettings.get_global_instance()
            )
            ...
            if result.is_success:
                data = result.data or {}
                entity_types: list[t.GeneralValueType] = data.get("entity_types", [])
    ...
    def handle_extract(
        self, args: dict[str, t.GeneralValueType] | None = None
    ) -> FlextResult[str]:
        ...
        try:
            ...
            entity_names_raw: list[t.GeneralValueType] = (
    ...
    def handle_pipeline(
        self,
        args: dict[str, t.GeneralValueType] | None = None,
    ) -> FlextResult[str]:
        ...
        try:
            ...
            (entity_names_raw if isinstance(entity_names_raw, list) else [])
            model_names_raw: list[t.GeneralValueType] = (
            ...
            if result.is_success:
                data = result.data or {}
                summary: dict[str, t.GeneralValueType] = data.get("summary", {})
    ...
    def handle_info(
        self, _args: dict[str, t.GeneralValueType] | None = None
    ) -> FlextResult[str]:
...
def discover() -> None:
    ...
    result: FlextResult[object] = cli_service.handle_discover()
...
def extract() -> None:
    ...
    cli_service = FlextDbtOracleWmsCliService()
    result: FlextResult[object] = cli_service.handle_extract()
    if result.is_failure:
        sys.exit(1)
...
def pipeline() -> None:
    ...
    cli_service = FlextDbtOracleWmsCliService()
    result: FlextResult[object] = cli_service.handle_pipeline()
    if result.is_failure:
        sys.exit(1)
...
def info() -> None:
    ...
    cli_service = FlextDbtOracleWmsCliService()
    result: FlextResult[object] = cli_service.handle_info()
    if result.is_failure:
        sys.exit(1)
