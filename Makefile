...
# flext-dbt-oracle-wms — selector-free generated project interface.
...
_builtin-help:
	@printf '%s\n' 'flext-dbt-oracle-wms [standalone]' '';
...
_builtin-self-check: _builtin_require_environment
	@set -eu; \
		gates="lint,pyrefly,mypy,pyright,silent-failure,deferred-self-reference,security,markdown,loc-cap,boundary,runtime-census,namespace,tier-whitelist,smells,codemod,layout,canonical-alias,direnv,index-declarations,duplication,budget"; \
		if [ "$(strip $(CI))" = "Y" ]; then \
			gates="lint,pyright,silent-failure,deferred-self-reference,security,markdown,loc-cap,boundary,runtime-census,namespace,tier-whitelist,smells,codemod,layout,canonical-alias,direnv,index-declarations,duplication,budget"; \
			printf 'INFO: CI=Y runs check gates: lint pyright silent-failure deferred-self-reference security markdown loc-cap boundary runtime-census namespace tier-whitelist smells codemod layout canonical-alias direnv index-declarations duplication budget\n'; \
		fi; \
		if [ -z "$$gates" ]; then \
